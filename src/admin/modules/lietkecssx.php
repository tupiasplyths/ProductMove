<?php
global $tencoso;
    if(isset($_POST['timcoso'])) {
    $tencoso = $_POST['tencoso'];
    }
    $sql_lietkesp = "SELECT * FROM sanpham, danhmuc WHERE sanpham.id_danhmuc = danhmuc.id AND sanpham.Cososanxuat LIKE '%" .$tencoso. "%' ORDER BY id_sanpham ASC";
    $query_lietkesp = mysqli_query($mysqli, $sql_lietkesp);
?>

<h3> Thống kê theo cơ sở sản xuất </h3>
