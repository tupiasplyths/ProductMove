<?php
global $tendaily;
    if(isset($_POST['timdaily'])) {
    $tendaily= $_POST['tendaily'];
    }
    $sql_lietkesp = "SELECT * FROM sanpham, danhmuc WHERE sanpham.id_danhmuc = danhmuc.id AND sanpham.Dailyphanphoi LIKE '%" .$tendaily. "%' ORDER BY id_sanpham ASC";
    $query_lietkesp = mysqli_query($mysqli, $sql_lietkesp);
?>

<h3> Thống kê theo đại lý phân phối </h3>
